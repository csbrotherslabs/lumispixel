...
class GetStartedPageTests(TestCase):
    def test_get_started_renders_new_choice_cards_for_anonymous_user(self):
        ...
        self.assertContains(response, 'id="get-started-photographer"')
        self.assertContains(response, 'id="get-started-photos"')
        self.assertContains(response, 'id="get-started-hire"')
        self.assertContains(response, "Get started as a photographer")
        self.assertContains(response, "View my photos")
        self.assertContains(response, "Find a photographer")
