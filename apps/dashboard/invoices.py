...
class InvoiceDeliveryError(ValidationError):
    """An invoice email could not be delivered."""
...
def send_invoice_email(invoice):
    """Deliver the persisted invoice document through Django's configured email backend."""
    ...
    owner = profile.user
    studio_name = profile.business_name or profile.display_name or owner.full_name or owner.email
    ...
    document = render_to_string(
        "photographer_workspace/invoices/print.html",
        {"invoice": invoice},
    )
    message.attach(f"{invoice.invoice_number}.html", document, "text/html")
    try:
        delivered = message.send(fail_silently=False)
    except Exception as exc:
        raise InvoiceDeliveryError(
            "The invoice email could not be delivered. The invoice was not marked as sent."
        ) from exc
    if delivered != 1:
        raise InvoiceDeliveryError(
            "The invoice email could not be delivered. The invoice was not marked as sent."
...
def save_invoice(profile, post, invoice=None, send=False):
    ...
    if send:
        ...
        invoice.sent_at = invoice.sent_at or timezone.now()
        invoice.full_clean()
