...
class PhotographerWorkspaceTests(TestCase):
    ...
    def test_gallery_settings_persist_validate_and_are_owner_scoped(self):
        ...
        self.assertContains(page, 'class="lp-gallery-settings lp-gallery-settings-modern"')
        self.assertContains(page, 'class="lp-settings-surface"')
        self.assertContains(page, "Client Experience")
    ...
    def test_gallery_pages_render_with_active_navigation_and_scoped_records(self):
        ...
        self.assertContains(photos_page, 'class="lp-photo-controlbar"')
        self.assertContains(photos_page, 'class="lp-photo-controlbar__filters"')
        self.assertContains(photos_page, "Search by file name")
        self.assertContains(photos_page, "Thumbnail size")
        self.assertContains(photos_page, ">Apply</button>")
        self.assertContains(photos_page, 'class="lp-gw-section-head"')
        store_page = self.client.get(reverse("photographer_workspace:gallery_workspace", args=[gallery.pk]), {"tab": "store"})
        self.assertContains(store_page, "Gallery Store")
        self.assertContains(store_page, 'class="lp-order-table"')
        analytics_page = self.client.get(reverse("photographer_workspace:gallery_analytics", args=[gallery.pk]))
        self.assertContains(analytics_page, 'class="lp-analytics lp-analytics-modern lp-analytics-tab-modern"')
        self.assertContains(analytics_page, "Gallery intelligence")
        self.assertContains(analytics_page, 'class="is-active" href="{}" aria-current="page"'.format(reverse("photographer_workspace:gallery_analytics", args=[gallery.pk])))
        self.assertContains(analytics_page, 'class="lp-analytics-compact-filters"')
        self.assertContains(analytics_page, 'class="lp-analytics-summary lp-analytics-summary--focused"')
        self.assertContains(analytics_page, "Analytics begin with your first client visit")
        self.assertNotContains(analytics_page, "Download Analytics")
        self.assertNotContains(analytics_page, "Traffic &amp; Devices")
    ...
    def test_gallery_create_edit_filters_and_bulk_actions(self):
        ...
        self.assertContains(create_page, "Build the client-facing foundation now, then add photos and delivery settings.")
        self.assertContains(create_page, 'class="workspace-form-page lp-container lp-add-lead lp-add-gallery lp-gallery-create-modern lp-clients-main-width"')
        self.assertContains(create_page, "Gallery builder")
        self.assertContains(create_page, 'class="lp-gallery-create-progress"')
        ...
        self.assertLess(create_page.content.index(b'gallery-details-title'), create_page.content.index(b'cover-image-title'))
        self.assertLess(create_page.content.index(b'cover-image-title'), create_page.content.index(b'publishing-access-title'))
        self.assertContains(create_page, 'aria-label="Live gallery preview"')
        self.assertContains(create_page, "Your delivery workspace is next")
    ...
    def test_activity_timeline_filters_exports_and_enforces_ownership(self):
        ...
        self.assertContains(page, "See the latest client and studio actions in one chronological feed.")
        self.assertContains(page, 'class="lp-activity-page lp-activity-page-modern"')
        self.assertContains(page, 'class="lp-activity-controlbar"')
        self.assertContains(page, "More filters")
        self.assertContains(page, "A visible timeline event.")
        self.assertContains(page, ">Details<i")
        self.assertNotContains(page, 'class="lp-activity-summary"')
    ...
    def test_client_detail_actions_tabs_alerts_and_isolation(self):
        ...
        self.client.post(reverse("photographer_workspace:client_archive_restore", args=[client.pk]), {"action": "archive"})
        client.refresh_from_db()
        self.assertEqual(client.status, Client.Status.ARCHIVED)
        self.client.post(reverse("photographer_workspace:client_archive_restore", args=[client.pk]), {"action": "restore"})
        client.refresh_from_db()
