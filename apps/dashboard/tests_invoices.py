...
from apps.clients.models import Client, ClientInvoice, InvoiceActivity, InvoicePayment
...
class InvoiceWorkspaceTests(TestCase):
    ...
    def payload(self, **changes):
        data = {
            ...
            "item_type[]": ["session", "travel"],
            "item_description[]": ["Portrait session", "Travel"],
            "item_quantity[]": ["2", "1"],
            "item_unit_price[]": ["125.55", "40.00"],
            "item_discount[]": ["10", "0"],
            "item_tax[]": ["8.25", "0"],
            "intent": "draft",
        }
    ...
    def test_create_page_exposes_required_builder_hooks(self):
    ...
    def test_edit_page_uses_existing_invoice_record(self):
    ...
    def test_invalid_item_is_rejected_without_partial_invoice(self):
        response = self.client.post(
            reverse("photographer_workspace:invoice_create"),
            self.payload(**{"item_quantity[]": ["-1", "1"]}),
        )
        self.assertEqual(response.status_code, 400)
        self.assertFalse(ClientInvoice.objects.exists())

    def test_send_records_activity_and_paid_invoice_cannot_be_voided(self):
        self.client.post(
            reverse("photographer_workspace:invoice_create"),
            self.payload(intent="send"),
        )
        ...
        self.client.post(
            reverse("photographer_workspace:invoice_action", args=[invoice.pk, "payment"]),
    ...
    def test_other_studio_cannot_view_invoice(self):
        ...
        response = self.client.get(reverse("photographer_workspace:invoice_view", args=[invoice.pk]))

        self.assertEqual(response.status_code, 404)

    def test_invoice_hub_empty_and_filtered_requests_render(self):
        ...
        filtered = self.client.get(url, {"q": "does-not-exist"})
        self.assertEqual(filtered.status_code, 200)
        self.assertNotContains(filtered, "INV-OPEN")

    def test_invoice_hub_filters_overdue_and_keeps_tenant_isolation(self):
        overdue = ClientInvoice.objects.create(
            photographer=self.profile,
            client=self.client_record,
        ...
        response = self.client.get(
            reverse("photographer_workspace:invoices"),
            {"status": "overdue"},
        )
