Warning: truncated output (original token count: 70709)
...
def gallery_preview(request, pk):
    ...
    context = _dashboard_context(r…20709 tokens truncated…rmation",
    ...
    for index, event in enumerate(events):
        ...
        if event["kind"] in ("booking", "consultation"):
            ...
            event["actions"] = [
                *([{"label": "Open Full Booking", "type": "link", "url": event["url"], "priority": "primary", "icon": "bi-arrow-right"}] if event["kind"] == "booking" else []),
...
def invitation_accept(request, token):
    ...
    if not request.user.is_authenticated:
        return render(request, "photographer_workspace/team/invitation_accept.html", {
            ...
            "signup_url": f'{reverse("accounts:client-signup")}?{urlencode({"next": request.get_full_path()})}',
        })
    ...
    if request.method == "POST" and email_matches:
        ...
        with transaction.atomic():
            ...
            if choice == "accept":
                ...
                if request.user.last_active_workspace != User.Workspace.PHOTOGRAPHER:
                    request.user.last_active_workspace = User.Workspace.PHOTOGRAPHER
                    request.user.save(update_fields=["last_active_workspace", "updated_at"])
