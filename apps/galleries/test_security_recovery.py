...
class MultipartSecurityRecoveryTests(TestCase):
    def setUp(self):
        ...
        self.owner = PhotographerProfile.objects.create(user=self.owner_user, onboarding_completed=True)
        self.other = PhotographerProfile.objects.create(user=self.other_user, onboarding_completed=True)
