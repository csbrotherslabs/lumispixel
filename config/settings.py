...
MIDDLEWARE = [
    ...
    "apps.core.observability.RequestIDMiddleware", "django.contrib.sessions.middleware.SessionMiddleware",
...
STATICFILES_STORAGE_BACKEND = os.getenv("DJANGO_STATICFILES_STORAGE", "whitenoise.storage.CompressedManifestStaticFilesStorage")
PRODUCTION_STATICFILES_STORAGE_BACKEND = "whitenoise.storage.CompressedManifestStaticFilesStorage"
if not DEBUG and STATICFILES_STORAGE_BACKEND != PRODUCTION_STATICFILES_STORAGE_BACKEND:
    raise RuntimeError("Production requires WhiteNoise CompressedManifestStaticFilesStorage.")
...
GALLERY_STORAGE_ENVIRONMENT = os.getenv("GALLERY_STORAGE_ENVIRONMENT", "dev" if DEBUG else "prod").strip().lower()
...
MEDIA_DELIVERY_BASE_URL = os.getenv("MEDIA_DELIVERY_BASE_URL", "https://media-dev.lumispixel.com" if GALLERY_STORAGE_ENVIRONMENT == "dev" else "https://media.lumispixel.com").rstrip("/")
...
if GALLERY_STORAGE_BACKEND == "b2" and not all([B2_ACCESS_KEY_ID, B2_SECRET_ACCESS_KEY, B2_BUCKET_NAME, B2_REGION, B2_ENDPOINT_URL]):
    raise RuntimeError("B2_ACCESS_KEY_ID, B2_SECRET_ACCESS_KEY, B2_BUCKET_NAME, B2_REGION, and B2_ENDPOINT_URL are required when GALLERY_STORAGE_BACKEND=b2.")
if not DEBUG:
    if GALLERY_STORAGE_BACKEND != "b2": raise RuntimeError("Production requires GALLERY_STORAGE_BACKEND=b2.")
    if GALLERY_STORAGE_ENVIRONMENT != "prod": raise RuntimeError("Production requires GALLERY_STORAGE_ENVIRONMENT=prod.")
    if not MEDIA_DELIVERY_BASE_URL.startswith("https://"): raise RuntimeError("Production MEDIA_DELIVERY_BASE_URL must use HTTPS.")
...
if not DEBUG:
    if not ALLOWED_HOSTS: raise RuntimeError("DJANGO_ALLOWED_HOSTS must not be empty in production.")
    if not CSRF_TRUSTED_ORIGINS or any(not origin.startswith("https://") for origin in CSRF_TRUSTED_ORIGINS): raise RuntimeError("DJANGO_CSRF_TRUSTED_ORIGINS must contain HTTPS origins in production.")
    if not PUBLIC_BASE_URL.startswith("https://"): raise RuntimeError("DJANGO_PUBLIC_BASE_URL must use HTTPS in production.")
    if not DATABASE_URL or not DATABASE_URL.lower().startswith(("postgresql://", "postgres://")): raise RuntimeError("Production DATABASE_URL must use PostgreSQL.")
    if not B2_ENDPOINT_URL.startswith("https://"): raise RuntimeError("Production B2_ENDPOINT_URL must use HTTPS.")
    if not EMAIL_HOST or not EMAIL_HOST_USER or not EMAIL_HOST_PASSWORD: raise RuntimeError("Production SMTP host and credentials are required.")
    if EMAIL_USE_TLS == EMAIL_USE_SSL: raise RuntimeError("Production email must enable exactly one of TLS or SSL.")

CELERY_BROKER_URL = os.getenv("CELERY_BROKER_URL", "redis://127.0.0.1:6379/0")
CELERY_RESULT_BACKEND = os.getenv("CELERY_RESULT_BACKEND", CELERY_BROKER_URL)
if not DEBUG:
    for setting_name, redis_url in {"CELERY_BROKER_URL": CELERY_BROKER_URL, "CELERY_RESULT_BACKEND": CELERY_RESULT_BACKEND}.items():
        if not redis_url.startswith("rediss://"): raise RuntimeError(f"{setting_name} must use rediss:// in production.")
...
if CELERY_TASK_SOFT_TIME_LIMIT <= 0 or CELERY_TASK_TIME_LIMIT <= CELERY_TASK_SOFT_TIME_LIMIT: raise RuntimeError("CELERY_TASK_TIME_LIMIT must be greater than CELERY_TASK_SOFT_TIME_LIMIT.")
CELERY_BROKER_CONNECTION_RETRY_ON_STARTUP = True
CELERY_BROKER_CONNECTION_RETRY = True
CELERY_BROKER_CONNECTION_MAX_RETRIES = int(os.getenv("CELERY_BROKER_CONNECTION_MAX_RETRIES", "20"))
if CELERY_BROKER_CONNECTION_MAX_RETRIES < 0: raise RuntimeError("CELERY_BROKER_CONNECTION_MAX_RETRIES must be non-negative.")
CELERY_VISIBILITY_TIMEOUT = int(os.getenv("CELERY_VISIBILITY_TIMEOUT", "3600"))
if CELERY_VISIBILITY_TIMEOUT <= CELERY_TASK_TIME_LIMIT: raise RuntimeError("CELERY_VISIBILITY_TIMEOUT must be greater than CELERY_TASK_TIME_LIMIT so Redis cannot redeliver a task while a worker may still be executing it.")
...
LOGGING = {
    ...
    "filters": {"safe_context": {"()": "apps.core.observability.SensitiveDataFilter"}},
    "formatters": {
        "production": {"format": "%(asctime)s %(levelname)s %(name)s request_id=%(request_id)s %(message)s"},
    },
    "handlers": {
        "console": {"class": "logging.StreamHandler", "filters": ["safe_context"], "formatter": "production"},
    },
